...
// Trigger server restart for .env load
...
// 15. EMAIL DIET PLAN
app.post("/api/email-plan", authMiddleware, async (req: any, res) => {
  try {
    ...
    if (!resendApiKey) {
      // If no API key is provided, log to console as simulation
      console.log(`[Email Simulation] Diet Plan PDF sent to ${userEmail}`);
    ...
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "AI Dietitian <onboarding@resend.dev>",
        to: userEmail,
        subject: "Your Personalized Diet Plan",
        html: "<p>Hello,</p><p>Please find attached your personalized diet plan generated by AI Dietitian.</p><p>Stay Healthy!</p>",
        attachments: [
          {
